...
describe('Soccer GET Endpoints', () => {
  ...
  test('GET /soccer - should return all soccer teams', async () => {
  ...
  test('GET /soccer/:id - valid ID returns a team', async () => {
    const res = await request(app).get('/soccer/64b1f2c2c3a71f1b7c2f0d13');
    expect(res.statusCode).toBe(200);
    expect(res.body.name).toBe('Test Team 1');
  });

  test('GET /soccer/:id - invalid ID returns 404', async () => {
    const nonExistentId = new ObjectId().toString();
    const res = await request(app).get(`/soccer/${nonExistentId}`);
    expect(res.statusCode).toBe(404);
    expect(res.body.message).toMatch(/not found/i);
  });

  test('GET /soccer - handles internal server error', async () => {
