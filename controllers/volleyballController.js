...
const getTeamById = async (req, res) => {
  try {
    ...
    const db = await mongoDb.getDb().collection('volleyballTeams').findOne({ _id: teamId });
    if (!db) {
...
const deleteTeam = async (req, res) => {
  try {
    const teamId = new objectId(req.params.id);
    const db = await mongoDb.getDb().collection('volleyballTeams').deleteOne({ _id: teamId });
